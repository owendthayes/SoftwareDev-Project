...
function updateUserRole($groupid, $username, $role) {
    ...
    if ($role !== 'admin' && isSoleAdmin($groupid, $username)) {
        echo "Cannot change role. You are the sole admin of the group.";
        mysqli_close($connection);
        exit;
    }
    
    // Prepare the SQL statement
    $stmt = mysqli_prepare($connection, "UPDATE group_participants SET gpermissions = ? WHERE groupid = ? AND username = ?");
